...
var IsWithinRange = (BigInteger number, BigInteger rangeStart, BigInteger rangeEnd) =>
{
...
var stepsRange = new Dictionary<BigInteger, List<SourceMap>>();

var SeedsList = new List<BigInteger>(ruleSets[1].Split(' ', StringSplitOptions.RemoveEmptyEntries).Select(x => BigInteger.Parse(x)));

var Seeds = new Dictionary<BigInteger, BigInteger>();
...
ruleSets.ForEach(x => {
  ...
  var rules = parts.Select(i=>i.Split(' ').Select(q => BigInteger.Parse(q)).ToList()).ToList();  
...
var currSeeds = new List<Seed>(Seeds.ToList().OrderBy(x => x.Key).Select(x => new Seed(x.Key, x.Value)));
  ...
  for(var i = 0; i < stepsRange.Count; i++)
  {
    ...
    foreach(var currSeed in currSeeds)
    {
      ...
      var matchedMaps = currentMaps.Where(map => 
        (currSeed.Min >= map.SourceMin && currSeed.Min <= map.SourceMax)||
        (currSeed.Max <= map.SourceMax && currSeed.Max >= map.SourceMin)||
        (currSeed.Min <= map.SourceMin && currSeed.Max >= map.SourceMax)).Distinct().ToList();
    ...
    //Console.WriteLine($"diffs: {newSeeds.Sum(x => (long)x.Diff)}");
    currSeeds = new List<Seed>(newSeeds.Distinct().OrderBy(x => x.Min));
